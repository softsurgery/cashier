...
- `src/pages/order/order.service.ts` has a `relactions` typo — use `relations`
- `order-product.ipc.ts` reuses `order:*` channels — must rename before registering
- User/RBAC backend exists in `shared/abstract-user-management/` but is not wired to DB or IPC
- Login is a client-side stub (`admin`/`admin`) — bcrypt is available but unused
- Folder `src/stores/table-zone-state /` has a trailing space in its name
